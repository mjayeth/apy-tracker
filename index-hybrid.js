/**
 * APY Tracker Main Script - Hybrid Version
 * 
 * DeFi vaults의 Net APY를 수집하는 메인 스크립트입니다.
 * Compound Blue vaults 2개는 Morpho API를 사용하고, 나머지 9개는 웹 스크래핑을 사용합니다.
 */

const vaults = require('./vaults');
const { scrapeAllVaultsNetApy } = require('./vercelScraper');
...
/**
 * 메인 실행 함수
 * 1. Compound Blue vaults 2개는 API로 조회
 * 2. 나머지 vaults 9개는 웹 스크래핑으로 조회
 * 3. 결과를 합쳐서 정렬 및 저장
 */
async function main() {
  console.log('🚀 Hybrid Net APY Tracker (API + Scraping) 시작...\n');
  ...
  try {
    ...
    // 나머지 vaults 9개는 웹 스크래핑으로 처리
    const nonCompoundVaults = vaults.filter(vault => vault.type !== 'compound_blue');
    console.log(`🌐 스크래핑으로 처리할 vaults: ${nonCompoundVaults.length}개`);
    ...
    console.log(`📈 성공: ${successfulVaults.length}개, 실패: ${errorVaults.length}개`);
    console.log(`🔗 API 성공: ${compoundResults.filter(v => v.netApy !== 'Error').length}개`);
    console.log(`🌐 스크래핑 성공: ${scrapingResults.filter(v => v.netApy !== 'Error').length}개`);
